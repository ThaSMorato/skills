...
- `docs/analysis/dependency-graph.md` (if present) — the measured import graph, for divergence. It is measured by the `architecture` skill's `import-graph.md`; do not re-extract it another way.
- `docs/analysis/reconcile.md` (if present) — drift since the contract was written: new edges the contract does not allow and new cycles, each with the commits that introduced it. On a re-run, each accepted item becomes a `fix` or an `amend` here. The reconciler measures; you own this file.
